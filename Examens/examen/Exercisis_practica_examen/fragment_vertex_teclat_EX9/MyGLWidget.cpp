...
void MyGLWidget::carregaShaders()
{
  ...
  blancnegre = glGetUniformLocation (program->programId(), "blneg");
  tipus = glGetUniformLocation (program->programId(), "tipus");
...
void MyGLWidget::modelTransformPatricio ()
{
  ...
  TG = glm::rotate(TG,gradesR,glm::vec3(0.0,1.0,0.0));
  TG = glm::translate(TG, -centrePatr);
  
  float vaca = 0.0;
  glUniform1f(tipus,vaca);
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva)
    Proj = glm::perspective(float(M_PI/3.0), 1.0f, 1.0f, 10.0f);
  else
...
void MyGLWidget::viewTransform ()
{
   // lookAt(OBS,VRP,UP)
  glm::mat4 View; 
  View = glm::lookAt (glm::vec3(-1,1,-1), glm::vec3(1,-0.5,0), glm::vec3(0,1,0));
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_X: {
      if(blneg==0.0) blneg=1.0;
      else blneg = 0.0;
      glUniform1f(blancnegre,blneg);
      break;
    }
