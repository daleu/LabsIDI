...
void MyGLWidget::modelTransformPatricio2()
{
  ...
  TG = glm::translate(TG, glm::vec3(-centrePatr[0],-(centrePatr[1]+altura),-centrePatr[2]));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva){
    ...
    float FOV;
    if(ra < 1){
      FOV = atan(tan(asin(radiEsc/(radiEsc*2+zoom)))/ra);
    }
    else FOV = asin(radiEsc/(radiEsc*2+zoom));
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  radiEsc = sqrt((pow(maxx-minx,2))+(pow(maxy*2-miny,2))+(pow(maxz-minz,2)))/2;
}
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  if (DoingInteractive == ZOOM)
  {
    ...
    float zoomx = zoom + (e->y() - yClick);
    std::cout << zoomx << std::endl;
    if(zoomx<=200 && zoomx>=-100){
      zoom += (e->y() - yClick);
      emit enviarFOV(zoom);
    }
    projectTransform ();
...
void MyGLWidget::applyzoom(int zoomi){
    makeCurrent();
    zoom = zoomi;
    projectTransform ();
    update();
