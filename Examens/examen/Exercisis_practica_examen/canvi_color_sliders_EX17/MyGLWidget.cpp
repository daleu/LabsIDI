...
void MyGLWidget::carregaShaders()
{
  ...
  llumView = glGetUniformLocation (program->programId(), "posFocus");
}
...
{
  glm::mat4 TG(1.f);  // Matriu de transformació
  TG = glm::translate(TG,glm::vec3(-2+movX,-1,0));
...
void MyGLWidget::projectTransform ()
{
  ...
  if (perspectiva){
    ...
    if(seguiment){
       if(ra < 1){
	FOV = atan(tan(M_PI/2)/ra);
      }
      else FOV = M_PI/2;
      std::cout << FOV << std::endl;
      Proj = glm::perspective(FOV, ra, 0.1f, 3.0f*radiEscena);
    } else {
      if(ra < 1){
	FOV = atan(tan(asin(radiEscena/(radiEscena*2)))/ra);
      }
      else FOV = asin(radiEscena/(radiEscena*2));
      std::cout << FOV << std::endl;
      Proj = glm::perspective(FOV, ra, radiEscena, 3.0f*radiEscena);
    }
  }
  else
...
void MyGLWidget::viewTransform ()
{
  ...
  if(seguiment){
    View = glm::lookAt (glm::vec3(-2+movX,1.5,-1), glm::vec3(2,1.25,1), glm::vec3(0,1,0));
  }else{
    View = glm::translate(glm::mat4(1.f), glm::vec3(0, 0, -2*radiEscena));
...
void MyGLWidget::calculaCapsaModel ()
{
  ...
  radiEscena = sqrt((pow(maxx-minx,2))+(pow(maxy+1,2))+(pow(4,2)))/2;
}
...
void MyGLWidget::llumTransform (){
  glm::vec3 posFocus = glm::vec3(-2+movX,0,0);
  glUniform3fv(llumView,GL_TRUE,&posFocus[0]);
}

void MyGLWidget::canviarCamara(bool clicat){
  makeCurrent();
  if(clicat) seguiment = true;
  else seguiment = false;
  viewTransform();
  projectTransform ();
  update();
}

void MyGLWidget::canviarBlau(int blau){
  makeCurrent();
  diff[2] = (double)blau/100;
  std::cout << diff[2] << std::endl;
  createBuffers();
  update();
}

void MyGLWidget::canviarRoig(int roig){
  makeCurrent();
  diff[0] = (double)roig/100;
  createBuffers();
  update();
}

void MyGLWidget::canviarVerd(int verd){
  makeCurrent();
  diff[1] = (double)verd/100;
  createBuffers();
  update();
}
