...
void MyGLWidget::projectTransform () 
{
  ...
  angle_prespectiva = asin(radiCapsa/(2*radiCapsa+provafov));
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_Z:{ // escalar a més gran
      provafov += 0.1;
      std::cout << "provafov: " << provafov << std::endl;
      projectTransform ();
      break;
    }
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e)
{
  ...
  {
    // Fem la rotació
    y += (e->x() - xClick) * M_PI / 180.0;
    x += (e->y() - yClick) * M_PI / 180.0;
    viewTransform ();
  }
